...
export class GroceryListService {
  ...
  async getListById(id: string, userId: string) {
    const groceryList = await this.repository.findListById(id, userId);

    if (!groceryList) {
      throw new AppError("Grocery list not found", HTTP_STATUS.NOT_FOUND);
    }
  ...
  async updateList(id: string, userId: string, data: UpdateGroceryListInput) {
    const result = await this.repository.updateList(id, userId, data);
  ...
  async deleteList(id: string, userId: string) {
    const result = await this.repository.deleteList(id, userId);
  ...
  async addItem(
    ...
    return this.repository.createItem(groceryListId, data);
  }
  ...
  async updateItem(
    ...
    return this.repository.findItemById(itemId, groceryListId);
  }

  async deleteItem(groceryListId: string, itemId: string, userId: string) {
    const groceryList = await this.repository.findListById(
      groceryListId,
      userId
    );
    ...
    const result = await this.repository.deleteItem(itemId, groceryListId);
